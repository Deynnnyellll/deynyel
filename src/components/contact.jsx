...
export const Contact = () => {
    ...
    return (
        <div id="contact" className="mt-20 w-full flex flex-col justify-center items-center gap-5">
            ...
            <form method='POST' action="https://getform.io/f/1cf65667-2d94-4be1-92f9-e28afb33a240" className='flex flex-col text-center items-center h-[28rem] md:h-[32rem] w-[85%] md:w-[65%] lg:w-[40%] rounded-sm outline outline-3 outline-darkTwo shadow-md shadow-darkTwo text-darkOne gap-4 p-4'>
                <div className="flex items-center gap-1 w-full bg-white py-1 px-2">
                    <FaUserCircle className="text-[#0F0F0F]" size={20}/>
                    <input type="text" className="h-full w-full p-3" placeholder="Name" name="name" required/>
                </div>

                <div className="flex items-center gap-1 w-full bg-white py-1 px-2">
                    <MdEmail className="text-[#00F0F0F]" size={20}/>
                ...
                <button type="onSubmit" className="text-[#0F0F0F] rounded-md bg-primary py-3 w-full hover:translate-y-[-0.5px] hover:shadow-white shadow-xs ease-in-out duration-200">
            ...
            <div className="flex flex-col items-center gap-3 mb-8">
                ...
                <div className="flex items-center justify-center text-[30px] gap-4">
                {
                    platforms.map(platform => (
                        <a key={platform.key} href={platform.link} target="_blank" className="hover:text-[#068FFF]">
                            {platform.item}
                        </a>
                    ))
                }
